---
name: data-search
description: 查询数据，包括日志、告警和安全事件。当用户需要根据时间范围、IP 地址、威胁等级或攻击结果搜索、过滤或检索安全数据时，应使用此技能。支持 SPL（搜索处理语言）语法。
---
...
## When to Use
- 用户询问日志、告警、安全事件
- 用户提及字段：src_ip、dst_ip、severity、attack_res、attack_direction
- 用户需要按时间范围、IP 段、威胁等级进行过滤
- 用户希望查看特定的安全事件
