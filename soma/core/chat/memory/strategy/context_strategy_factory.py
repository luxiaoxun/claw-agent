...
from soma.core.chat.memory.strategy.context_strategy import (ContextStrategy, RoundBasedContextStrategy,
                                                        TokenBasedContextStrategy, MessageCountBasedContextStrategy,
                                                        SemanticMemoryContextStrategy, HybridContextStrategy)
...
class ContextStrategyFactory:
    ...
    def create_from_settings(cls) -> ContextStrategy:
        """根据 settings 配置创建策略"""
        strategy_type = getattr(settings, 'CONTEXT_STRATEGY', 'token')

        strategy_configs = {
            "round": {
                "max_rounds": getattr(settings, 'MSG_MAX_HISTORY_LENGTH', 10)
            },
            "token": {
