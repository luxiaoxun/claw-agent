...
INJECT_PATTERNS = [
    ...
    (r"(你是|我扮演|I am|pretend|角色扮演).{0,30}(AI|助手|assistant|没有限制|无约束|no restrictions)", "角色扮演绕过",
     True),
...
CREDENTIAL_PATTERNS = [
    ...
    (r"(?i)(slack|discord|telegram|钉钉|飞书)[_-]?(token|key|secret|webhook)\s*[:=]\s*['\"]?[^'\"]{10,}", "IM 平台凭据",
     "HIGH"),
...
def analyze_semantic_risk(text: str) -> list[dict]:
    ...
    # 社会工程学诱导
    social_engineering = [
        (r"(请|please).{0,20}(信任|trust|允许|allow|授权|authorize|批准|approve)", "授权诱导",
         "提示用户信任/授权操作，降低警惕"),
        ...
        (r"(复制|粘贴|运行|copy|paste|run).{0,15}(以下|this|the).{0,10}(命令|代码|脚本|command|code|script)",
         "直接执行诱导", "诱导用户直接执行来源不明的代码"),
...
def run_full_audit(skill_path: str = None, skill_text: str = None) -> dict:
    ...
    if not has_name:
        all_issues.append(
            {"line": 0, "type": "缺少 name 字段", "detail": "SKILL.md 必须包含 name 字段", "severity": "CRITICAL"})
    if not has_description:
        all_issues.append(
            {"line": 0, "type": "缺少 description 字段", "detail": "SKILL.md 应包含 description 字段以说明用途",
             "severity": "MEDIUM"})
    ...
    deps_matches2 = re.findall(r"(?:^\s*|pip install |uv add |requirements|dependencies)[:\s]+([a-zA-Z0-9_\.-]+)", text,
                               re.MULTILINE)
    ...
    return {
        ...
        "breakdown": {
            "A_结构与Unicode": sum(
                1 for i in unique_issues if "Unicode" in i["type"] or "控制字符" in i["type"] or "结构" in i["type"]),
            ...
            "F_依赖安全": sum(1 for i in unique_issues if
                              "Typosquatting" in i["type"] or "版本未锁定" in i["type"] or "Git.*来源" in i[
                                  "type"] or "非标准包源" in i["type"] or "已知漏洞" in i["type"]),
            "G_权限最小化": sum(1 for i in unique_issues if "权限" in i["type"]),
            "H_混淆隐藏": sum(
                1 for i in unique_issues if "混淆" in i["type"] or "隐藏" in i["type"] or "Base64" in i["type"]),
...
def detect_malicious_skill(
    ...
    if issues:
        ...
        for issue in issues:
            ...
            lines_output.append(
                f"| {issue.get('severity', '?')} | {line_no} | {issue.get('type', '?')} | {issue.get('detail', '?')} |")
    else:
...
def main():
    parser = argparse.ArgumentParser(
        description="Skill 安全审计工具 - 检测恶意代码和风险行为",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
示例:
  %(prog)s --path /path/to/skill-directory
  %(prog)s --path ./SKILL.md
  %(prog)s --text "name: test\\ndescription: demo"
  %(prog)s --path ./my-skill --json
        """
    )

    input_group = parser.add_mutually_exclusive_group(required=True)
    input_group.add_argument("--path", "-p", type=str, help="Skill 文件或目录路径")
    input_group.add_argument("--text", "-t", type=str, help="Skill 内容字符串")
    ...
    # 执行审计
    result = detect_malicious_skill(
        skill_path=args.path,
        skill_text=args.text
    )
    ...
    if args.json:
        ...
        # 按行输出 result 中的 report
        if "error" in result:
            print(f"错误: {result['error']}")
        else:
            report = result.get("report", "无法生成报告")
            # 按行输出
