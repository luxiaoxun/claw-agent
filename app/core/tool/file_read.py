...
class FileReadTool(BaseTool):
    """
    Enhanced file reading tool supporting multiple formats:
    - Text files: returns original content
    ...
    name: str = "file_read"
    description: str = """Read file content with support for multiple formats:
    ...
    skills_dir: Path = Field(description="Skills directory path")
    ...
    def __init__(self, workspace_dir: str, **kwargs):
        ...
        logger.info(f"FileReadTool initialized with workspace_dir: {workspace_path.resolve()}")

    def _resolve_path(self, path: str, skill_name: Optional[str] = None) -> Optional[Path]:
        """
        ...
        # 2. Use skill_name
        if skill_name:
            candidate = self.skills_dir / skill_name / path
            if candidate.exists():
                logger.debug(f"Found path with skill_name: {candidate}")
                return candidate
            # Try with just filename
            candidate = self.skills_dir / skill_name / Path(path).name
            if candidate.exists():
    ...
    def _find_similar_files(self, directory: Path, filename: str, max_suggestions: int = 3) -> List[str]:
    ...
    def _read_image(self, filepath: Path, mime_type: str) -> str:
    ...
    def _read_pdf(self, filepath: Path, mime_type: str) -> str:
    ...
    def _run(self, path: str, skill_name: Optional[str] = None) -> str:
        ...
        try:
            # Resolve file path
            file_path = self._resolve_path(path, skill_name)

            if not file_path or not file_path.exists():
                # Try to find similar files
                search_dir = None
                if skill_name:
                    search_dir = self.skills_dir / skill_name
                elif self.workspace_dir.exists():
                    search_dir = self.workspace_dir

                if search_dir and search_dir.exists():
                    suggestions = self._find_similar_files(search_dir, Path(path).name)
                    if suggestions:
            ...
            if mime_type in IMAGE_MIME_TYPES:
                ...
                return self._read_image(file_path, mime_type)

            # Handle PDFs
            if mime_type == 'application/pdf':
                logger.info(f"Processing PDF file: {file_path}")
                return self._read_pdf(file_path, mime_type)
    ...
    async def _arun(self, path: str, skill_name: Optional[str] = None) -> str:
        ...
        logger.debug(f"Async read request for path: {path}, skill_name: {skill_name}")
...
if __name__ == "__main__":
    ...
    tool = FileReadTool(workspace_dir=WORK_DIR)

    # Read text file (relative path to workspace)
    result = tool._run(path="data-search/references/spl_syntax.md")
    print(result)

    # Read skill file
    result = tool._run(path="SKILL.md", skill_name="data-search")
    print(result)
    ...
    # Read PDF (returns base64)
    result = tool._run(path="document.pdf")
    print(result)
